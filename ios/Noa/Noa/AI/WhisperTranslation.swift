...
class WhisperTranslation: NSObject {
  ...
  // Add a new property to keep track of tasks that need to be retried due to rate limit errors
  private var _rateLimitedTasks: Set<Int> = []
  ...
  public init(configuration: NetworkConfiguration) {
    ...
    case .backgroundUpload:
      ...
      // with a file)
      _tempFileURL = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
        .appendingPathComponent(UUID().uuidString)
      fallthrough
    case .backgroundData:
      // Configure a URL session that supports background transfers
      let configuration = URLSessionConfiguration.background(
        withIdentifier: "Whisper-\(UUID().uuidString)")
  ...
  public func translate(
    fileData: Data, format: AudioFormat, completion: @escaping (String, AIError?) -> Void
  ) {
    ...
    formData.append(
      "Content-Disposition: form-data; name=\"file\"; filename=\"audio.\(format.rawValue)\"\r\n"
        .data(using: .utf8)!)
    ...
    let task =
      _tempFileURL == nil
      ? _session.dataTask(with: request)
      : _session.uploadTask(with: request, fromFile: _tempFileURL!)
  ...
  private func extractContent(from data: Data) -> (AIError?, String?) {
    do {
      ...
      if let response = json as? [String: AnyObject] {
        ...
        } else if let transcript = response["text"] as? String {
  ...
  // Add a new function to handle retries with exponential backoff
  public func retryWithExponentialBackoff(
    maxRetries: Int, delay: TimeInterval, task: URLSessionTask,
    completion: @escaping (String, AIError?) -> Void
  ) {
    print("[WhisperTranslation] Retrying task with exponential backoff...")
    if maxRetries > 0 {
      print("[WhisperTranslation] Waiting for \(delay) seconds before retrying...")
      DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
        print("[WhisperTranslation] Resuming task...")
        task.resume()
      }
      _completionByTask[task.taskIdentifier] = { result, error in
        if let error = error as? AIError, case .rateLimitExceeded = error {
          print("[WhisperTranslation] Task was rate limited again, retrying...")
          self.retryWithExponentialBackoff(
            maxRetries: maxRetries - 1, delay: delay * 2, task: task, completion: completion)
        } else {
          print("[WhisperTranslation] Task completed successfully or with non-rate limit error.")
          completion(result, error)
        }
      }
    } else {
      print("[WhisperTranslation] Maximum retries reached, returning rate limit error.")
      completion("", AIError.rateLimitExceeded)
    }
  }
}
...
extension WhisperTranslation: URLSessionDataDelegate {
  public func urlSession(
    _ session: URLSession, dataTask: URLSessionDataTask, didBecome streamTask: URLSessionStreamTask
  ) {
  ...
  public func urlSession(
    ...
    if let error = error {
      print(
        "[WhisperTranslation] URLSessionDataTask failed to complete: \(error.localizedDescription)")
    } else {
      // Error == nil should indicate successful completion
      print("[WhisperTranslation] URLSessionDataTask finished")
    }

    print("[WhisperTranslation] _rateLimitedTasks = \(_rateLimitedTasks)")
    ...
    DispatchQueue.main.async { [weak self] in
      guard let self = self else { return }
      if let completion = self._completionByTask[task.taskIdentifier] {
        // Check if the task was rate limited
        if self._rateLimitedTasks.contains(task.taskIdentifier) {
          print("[WhisperTranslation] Task was rate limited, initiating retry...")
          // Remove the task from the set of rate limited tasks
          self._rateLimitedTasks.remove(task.taskIdentifier)
          // Retry the task with exponential backoff
          self.retryWithExponentialBackoff(
            maxRetries: 3, delay: 1.0, task: task, completion: completion)
        } else {
          print(
            "[WhisperTranslation] Task completed with error: \(error?.localizedDescription ?? "Unknown error")"
          )
          completion("", AIError.clientSideNetworkError(error: error))
          self._completionByTask.removeValue(forKey: task.taskIdentifier)
        }
  ...
  public func urlSession(
    ...
    if response.statusCode == 429 {
      ...
      print("[WhisperTranslation] URLSessionDataTask rate limit exceeded")
      _rateLimitedTasks.insert(dataTask.taskIdentifier)
