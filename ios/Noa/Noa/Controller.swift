...
class Controller: ObservableObject, LoggerDelegate, DFUServiceDelegate, DFUProgressDelegate {
  ...
  private class FPGAUpdateState: Equatable {
    ...
    init(maximumDataLength: Int, rescaleUpdatePercentage: Bool) {
      ...
      print(
        "[Controller] FPGA update: image=\(image.count) bytes, chunkSize=\(chunkSize), chunks=\(chunks), maximumDataLength=\(maximumDataLength)"
      )
    }
  ...
  private class ScriptTransmissionState: Equatable {
    ...
    static func == (
      lhs: Controller.ScriptTransmissionState, rhs: Controller.ScriptTransmissionState
    ) -> Bool {
      return ObjectIdentifier(lhs) == ObjectIdentifier(rhs)
    }
  }
  ...
  private static let _firmwareURL = Bundle.main.url(
    forResource: "monocle-micropython-v23.248.0754", withExtension: "zip")!
  ...
  private let _monocleFormat = AVAudioFormat(
    commonFormat: .pcmFormatInt16, sampleRate: 8000, channels: 1, interleaved: false)!
  ...
  init(settings: Settings, messages: ChatMessageStore) {
    ...
    _dfuInitiator = DFUServiceInitiator(
      queue: _bluetoothQueue, delegateQueue: .main, progressQueue: .main
    ...
    _monocleBluetooth.discoveredDevices.sink {
      [weak self] (devices: [(deviceID: UUID, rssi: Float)]) in
    ...
    _monocleBluetooth.dataReceived.sink {
      [weak self] (received: (characteristic: CBUUID, value: Data)) in
      DispatchQueue.main.async { [weak self] in
        guard let self = self else { return }
    ...
    _dfuBluetooth.peripheralConnected.sink { [weak self] (_: UUID) in
      ...
      DispatchQueue.main.async { [weak self] in
        ...
        if case let .initiateDFUAndWaitForDFUTarget(
          rescaleUpdatePercentage: rescaleUpdatePercentage) = _state {
          transitionState(
            to: .performDFU(
              peripheral: peripheral, rescaleUpdatePercentage: rescaleUpdatePercentage))
        } else {
  ...
  private func transitionState(to newState: State) {
    ...
    case .enterRawREPL(let didFinishDFU):
      ...
      DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
        guard let self = self else { return }
        guard case .enterRawREPL = _state else { return }
    ...
    case .waitingForRawREPL(let didFinishDFU):
      _matcher = nil
    ...
    case .performDFU(let peripheral, rescaleUpdatePercentage: _):
    ...
    case .initiateFPGAUpdate(let maximumDataLength, let rescaleUpdatePercentage):
      monocleState = .updatingFPGA
      updateProgressPercent = rescaleUpdatePercentage ? 50 : 0
      _matcher = nil
      let updateState = FPGAUpdateState(
        maximumDataLength: maximumDataLength, rescaleUpdatePercentage: rescaleUpdatePercentage)
      print("[Controller] Updating FPGA...")
      transmitFPGADisableAndErase()
      transitionState(to: .waitForFPGAErased(updateState: updateState))

    case .waitForFPGAErased:
      _matcher = nil

    case .sendNextFPGAImageChunk(let updateState):
  ...
  private func handleSerialDataReceived(_ receivedValue: Data) {
    ...
    case .waitingForFirmwareVersion(let didFinishDFU):
      onWaitForFirmwareVersionString(receivedString: str, didFinishDFU: didFinishDFU)

    case .waitingForFPGAVersion(let didFinishDFU):
    ...
    case .waitForFPGAErased(let updateState):
  ...
  private func onWaitForFirmwareVersionString(receivedString str: String, didFinishDFU: Bool) {
    ...
    if _receivedVersionResponse.contains("\u{4}>") {
      ...
      if !_receivedVersionResponse.contains("Error") && parts[0].count >= 3
        && parts[0].starts(with: "OK") {
  ...
  private func onWaitForFPGAVersionString(receivedString str: String, didFinishDFU: Bool) {
    ...
    if _receivedVersionResponse.contains("\u{4}>") {
      ...
      if !_receivedVersionResponse.contains("Error") && parts[0].count >= 3
        && parts[0].starts(with: "OK") {
        let str = parts[0].replacingOccurrences(of: "b'", with: "").replacingOccurrences(
          of: "'", with: "")  // strip out b''
  ...
  private func updateMonocleOrProceedToRun(didFinishDFU: Bool) {
    if _currentFirmwareVersion != _requiredFirmwareVersion {
      ...
      print(
        "[Controller] Firmware update needed. Current version: \(_currentFirmwareVersion ?? "unknown")"
      )
      ...
      // Do update
      transitionState(
        to: .initiateDFUAndWaitForDFUTarget(
          rescaleUpdatePercentage: rescaleFirmwareUpdatePercentage))
        ...
        if let maximumDataLength = _monocleBluetooth.maximumDataLength, maximumDataLength > 100 {
          ...
          DispatchQueue.main.async { [weak self] in  // back to main thread...
            self?.transitionState(
              to: .initiateFPGAUpdate(
                maximumDataLength: maximumDataLength,
                rescaleUpdatePercentage: rescaleFPGAUpdatePercentage))
          }
        } else {
          // We don't know the MTU size or it is unreasonably small, cannot update, proceed otherwise
          let mtuSize =
            _monocleBluetooth.maximumDataLength == nil
            ? "unknown" : "\(_monocleBluetooth.maximumDataLength!)"
  ...
  private func transmitNextFPGAImageChunk(updateState: FPGAUpdateState) {
    ...
    let chunkStart = updateState.image.index(
      updateState.image.startIndex, offsetBy: updateState.chunk * updateState.chunkSize)
    let chunkEnd = updateState.image.index(
      updateState.image.startIndex,
      offsetBy: min(updateState.image.count, (updateState.chunk + 1) * updateState.chunkSize))
    ...
    let progress = min(
      100, 100 * updateState.chunk * updateState.chunkSize / updateState.image.count)
  ...
  private func onScriptTransmitted(
    receivedString str: String, scriptTransmissionState: ScriptTransmissionState
  ) {
    if _matcher == nil {
      _matcher = Util.StreamingStringMatcher(lookingFor: "OK\u{4}\u{4}>")
    }
  ...
  private func transmitFPGADisableAndErase() {
    ...
    transmitPythonCommand(
      "import ubinascii,update,device,bluetooth,fpga;fpga.run(False);update.Fpga.erase()")
  }
  ...
  private func transmitNextScript(scriptTransmissionState: ScriptTransmissionState) {
    ...
    guard
      let command = "f=open('\(filename)','w');f.write('''\(contents)''');f.close()".data(
        using: .utf8)
    else {
    ...
    if command.starts(with: "ast:") || command.starts(with: "ien:") {
      ...
      if command.starts(with: "ien:") {
        ...
        if !_imageData.isEmpty {
          ...
          ) { [weak self] (response: String, b: String, error: AIError?) in
              print("[Controller] Vision received: \(response) \(b)")
              self?.printToChat(b, as: .assistant)
              if error != nil {
  ...
  private func sendTextToMonocleInChunks(text: String, isError: Bool) {
    _bluetoothQueue.async { [weak _monocleBluetooth] in
      guard let _monocleBluetooth = _monocleBluetooth,
        var chunkSize = _monocleBluetooth.maximumDataLength
      else {
        return
      }
  ...
  private func sendImageToMonocleInChunks(image: UIImage) {
    ...
    guard bitmap.count == expectedSize else {
      print(
        "[Controler] Error: RGB343 image is not the expected size (got \(bitmap.count) bytes instead of \(expectedSize))"
      )
      return
    }
  ...
  private func playReceivedAudio(_ pcmBuffer: AVAudioPCMBuffer) {
    if let audioConverter = _audioConverter {
      ...
      let outputBuffer = AVAudioPCMBuffer(
        pcmFormat: _playbackFormat, frameCapacity: pcmBuffer.frameLength * 48 / 8)!
      ...
      audioConverter.convert(
        ...
        withInputFrom: {
          (
            ...
            outError: UnsafeMutablePointer<AVAudioConverterInputStatus>
          ) -> AVAudioBuffer? in
  ...
  public func dfuProgressDidChange(
    ...
    if case let .performDFU(peripheral: _, rescaleUpdatePercentage: rescaleUpdatePercentage) =
      _state {
